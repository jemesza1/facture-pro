...
function loadData(){try{const r=localStorage.getItem(STORAGE_KEY);if(r){const d=JSON.parse(r);state={...state,...d};}}catch(e){}state.clients=(state.clients||[]).map(function(c){return Object.assign({nin:'',nis:'',rc:'',ai:'',email:'',address:'',nif:'',phone:''},c);});state.invoices=(state.invoices||[]).map(function(i){return Object.assign({paymentMode:'virement'},i);});state.company=Object.assign({},defaultCompany,state.company||{});if(!state.clients.length&&!state.invoices.length)seedDemoData();}
...
function seedDemoData(){state.clients=[{id:'c1',name:'SARL Atlas Services',email:'contact@atlas.dz',address:'45 Bd Mohamed V\n16000 Alger',nif:'099999999999999',nis:'099888777666555',rc:'16/00-1234567B21',phone:'021 00 00 01'},{id:'c2',name:'EURL Sahara Tech',email:'info@sahara.dz',address:'8 Rue de la Liberté\n31000 Oran',nif:'088888888888888',nis:'088777666555444',rc:'31/00-7654321B19',phone:'041 00 00 02'},{id:'c3',name:'SPA Numidia Trading',email:'admin@numidia.dz',address:'22 Av de l\'Indépendance\n25000 Constantine',nif:'077777777777777',nis:'077666555444333',rc:'25/00-2468013B20',phone:'031 00 00 03'}];const today=new Date();const d=o=>{const dt=new Date(today);dt.setDate(dt.getDate()+o);return dt.toISOString().slice(0,10);};state.invoices=[{id:'inv1',number:'FAC-2026-001',clientId:'c1',template:'moderne',date:d(-25),dueDate:d(-10),status:'enretard',items:[{description:'Audit stratégique Q1',qty:1,unitPrice:250000,tva:19},{description:'Accompagnement (5 jours)',qty:5,unitPrice:45000,tva:19}],notes:'Paiement par virement sous 15 jours.'},{id:'inv2',number:'FAC-2026-002',clientId:'c2',template:'premium',date:d(-12),dueDate:d(3),status:'envoyee',items:[{description:'Développement module facturation',qty:1,unitPrice:480000,tva:19}],notes:''},{id:'inv3',number:'FAC-2026-003',clientId:'c3',template:'classique',date:d(-5),dueDate:d(25),status:'payee',items:[{description:'Création identité visuelle',qty:1,unitPrice:180000,tva:19}],notes:'Merci.'},{id:'inv4',number:'FAC-2026-004',clientId:'c1',template:'nature',date:d(-2),dueDate:d(28),status:'brouillon',items:[{description:'Conseil en organisation',qty:3,unitPrice:55000,tva:19}],notes:''}];state.nextInvoiceNumber=5;saveData();}
