...
function renderPage(){const c=document.getElementById('main-content');if(!c)return;updateOverdue();if(state.currentPage==='dashboard')c.innerHTML=renderDashboard();else if(state.currentPage==='invoices')c.innerHTML=renderInvoices();else if(state.currentPage==='clients')c.innerHTML=renderClients();else if(state.currentPage==='templates')c.innerHTML=renderTemplates();else if(state.currentPage==='settings')c.innerHTML=renderSettings();else if(state.currentPage==='help')c.innerHTML=renderHelp();else if(state.currentPage==='terms')c.innerHTML=renderTerms();else c.innerHTML='<p class="text-slate-500">'+esc(t('ui.notFound'))+'</p>';try{lucide.createIcons();}catch(e){}try{if(typeof paintBackupNotice==='function')paintBackupNotice();}catch(e){}animateCounters();}
