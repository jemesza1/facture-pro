...
(function(){
  ...
  },
  {
    ...
    h1fr: "La facture d’acompte",
    h1ar: 'فاتورة التسبيق',
    leadfr: "« 30 % à la commande, le reste à la livraison » se dit dans tous les ateliers du pays. Cela fait deux factures, pas une : l’acompte, puis le solde qui le déduit.",
    leadar: '«30% عند الطلب والباقي عند التسليم» تُقال في كلّ ورشة في البلد. وهذا يعني فاتورتين لا واحدة: التسبيق، ثمّ التصفية التي تخصمه.',
    body: [
      {h: ['Deux factures pour une commande', 'فاتورتان لطلبية واحدة'],
       p: [['L’acompte n’est pas un reçu : c’est une facture, avec son numéro dans votre série, sa TVA et sa date. Elle est encaissée avant que rien ne soit livré. À la livraison vient la seconde, la facture de solde, qui porte la commande entière et déduit ce qui a déjà été facturé.',
            'التسبيق ليس وصلاً: هو فاتورة، برقمها في سلسلتك، وبرسمها وتاريخها. تُحصَّل قبل أن يُسلَّم شيء. وعند التسليم تأتي الثانية، فاتورة التصفية، تحمل الطلبية كاملة وتخصم ما سبقت فوترته.'],
           ['La commande elle-même reste un brouillon : elle n’a été envoyée à personne et n’entre donc pas dans les comptes. Les deux seules factures émises sont l’acompte et le solde, et leur somme fait exactement la commande.',
            'أمّا الطلبية نفسها فتبقى مسودّة: لم تُرسَل لأحد، فلا تدخل الحسابات. والفاتورتان الوحيدتان الصادرتان هما التسبيق والتصفية، ومجموعهما يساوي الطلبية بالضبط.']]},
      {h: ['La TVA se découpe par taux', 'الرسم يُقسَّم حسب النسبة'],
       p: [['C’est l’erreur que font presque tous les modèles gratuits. Un acompte de 30 % sur une commande qui mélange du 19 % et du 9 % n’est pas « 30 % à 19 % ». Il se découpe : 30 % de la base à 19, 30 % de la base à 9, chacune sur sa ligne. Une seule ligne au taux le plus élevé fait payer au client une taxe qu’il ne doit pas, et la déclaration ne tombe plus.',
            'هذا هو الخطأ الذي تقع فيه أغلب النماذج المجانية. تسبيق بنسبة 30% على طلبية تخلط 19% و9% ليس «30% بنسبة 19%». بل يُقسَّم: 30% من الوعاء بـ19، و30% من الوعاء بـ9، كلّ في سطره. وسطر واحد بالنسبة الأعلى يجعل العميل يدفع رسماً لا يستحقّه، ولا يعود التصريح مضبوطاً.']]},
      {h: ['Ce que la facture d’acompte doit porter', 'ما يجب أن تحمله فاتورة التسبيق'],
       ul: [
         ['Le mot « acompte » en clair, et le pourcentage ou le montant convenu', 'كلمة «تسبيق» صراحةً، والنسبة أو المبلغ المتّفق عليه'],
         ['La référence de la commande ou du devis auquel il se rapporte', 'مرجع الطلبية أو عرض السعر الذي يتعلّق به'],
         ['Un numéro pris dans votre série de factures, pas une numérotation à part', 'رقماً من سلسلة فواتيرك، لا ترقيماً منفصلاً'],
         ['La TVA ligne par ligne, au taux de ce qui est commandé', 'الرسم سطراً بسطر، بنسبة ما هو مطلوب'],
         ['Le droit de timbre si l’acompte est réglé en espèces', 'حقّ الطابع إن دُفع التسبيق نقداً'],
         ['Les mentions habituelles : NIF, NIS, RC, AI, et le montant en lettres', 'البيانات المعتادة: NIF وNIS وRC وAI، والمبلغ بالحروف'],
       ]},
      {h: ['Le solde, et le piège du double comptage', 'التصفية، وفخّ الحساب المزدوج'],
       p: [['Facturer l’acompte puis facturer la totalité, c’est déclarer deux fois le même chiffre d’affaires et la même taxe. La facture de solde doit donc porter la commande entière, et la déduction, ligne à ligne, taux par taux. Le net à payer est alors ce qui reste réellement dû.',
            'أن تفوتر التسبيق ثمّ تفوتر المجموع كلّه يعني التصريح مرّتين بنفس رقم الأعمال ونفس الرسم. فوجب أن تحمل فاتورة التصفية الطلبية كاملة ومعها الخصم، سطراً بسطر ونسبةً بنسبة. عندئذ يكون الصافي المستحقّ هو الباقي فعلاً.'],
           ['Réglé en espèces, chaque versement porte son propre droit de timbre : deux paiements, deux timbres. Ce n’est pas une double taxation, c’est un droit assis sur l’encaissement.',
            'وإن كان الدفع نقداً فلكلّ تسديد حقّ طابعه: دفعتان، طابعان. وليس هذا ازدواجاً ضريبياً، بل حقّ يقوم على التحصيل.']]},
      {h: ['Le faire en une minute', 'اعملها في دقيقة'],
       p: [['Préparez la commande en brouillon, ouvrez son aperçu, et demandez la facture d’acompte : le pourcentage est le seul chiffre à saisir, le découpage de la TVA se fait tout seul. À la livraison, le bouton « facture de solde » reprend la commande et déduit l’acompte. Les deux se téléchargent en PDF ou en Excel, sans compte et sans abonnement.',
            'حضّر الطلبية كمسودّة، وافتح معاينتها، واطلب فاتورة التسبيق: النسبة هي الرقم الوحيد الذي تُدخله، وتقسيم الرسم يتمّ وحده. وعند التسليم يأخذ زرّ «فاتورة التصفية» الطلبية ويخصم التسبيق. وتُحمَّل الاثنتان بصيغة PDF أو Excel، بلا حساب وبلا اشتراك.']]},
    ],
    extra: dl('acompte', "Télécharger le modèle de facture d'acompte", 'حمّل نموذج فاتورة التسبيق'),
  },
  {
